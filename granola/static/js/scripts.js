	...
	$.ajax({
        ...
        success: function (json) {
			if (json.quantity_error) {
				$('#results').html( "<div style='color:red' class='alert-box' data-alert>" + json.quantity_error + "</div><p></p>");
			} else {
				console.log('success');
        ...
        error: function (xhr, errmsg) {
            $('#results').html("<h6 data-alert>Oops! The error has happened:" + errmsg + "</h6>");
        }
    })
})


$('.reduce_from_cart_button').click(function (e) {
	e.preventDefault();
	let id = $(this).data('id');
	let data = {
		csrfmiddlewaretoken: $('input[name=csrfmiddlewaretoken]').val(),
	...
	$.ajax({
		...
		success: function (json) {
			console.log('success');
			alert('The item was removed from cart');
			location.reload();
		},
		error: function (xhr, errmsg) {
...
$('.update_profile').keyup(function (event){
    if (event.code === 'Enter'){
        $('.send').trigger('click');
    }
})
